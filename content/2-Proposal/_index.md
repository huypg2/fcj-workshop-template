...
- **High Availability & Routing:** An Application Load Balancer (ALB) in Public Subnets intercepts public HTTP traffic on port 80, routing API queries (/api/*) to the backend API Gateway on port 8080 and serving static files via the Frontend Nginx containers.
- **Real-time Alerting:** A CloudWatch Alarm triggers an Amazon SNS topic to dispatch email alerts to administrators if traffic exceeds the defined threshold, such as RequestCount > 100 requests/minute.
...
- **Amazon VPC:** Drives network virtualization with 4 Subnets, Route Tables, Internet Gateway, and NAT Gateway.
...
- **Backend Task:** A Multi-Container Task enclosing the Java/Spring Boot combined microservices, including API Gateway, Account, Payment, Transaction services, and a Redis sidecar cache communicating on localhost.
- **Database & Backup:** RDS PostgreSQL instance stores user balances and transactions. Manual snapshots are captured and exported to the S3 bucket, encrypted via pg-s3-export-key.
...
- **Container Registry & Compute:** AWS ECR with 2 private repositories, AWS ECS Fargate cluster, Task execution role, and Task Definitions with multi-container definitions.
- **Secure Networking:** Custom VPC with 2 Public and 2 Private Subnets, NAT Gateway, Route Tables routing outbound traffic, and Security Groups enforcing least privilege by restricting DB ingress to EC2 Bastion and ECS tasks.
